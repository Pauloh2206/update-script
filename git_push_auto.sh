...
# 1. CONFIGURAR REMOTO (URL) - MODIFICADO/CORRIGIDO
# ----------------------------------------------------------
REMOTE_URL=$(git remote get-url origin 2>/dev/null)

# Se já existe um remoto, pergunta se deseja trocá-lo
if [ -n "$REMOTE_URL" ]; then
    ...
    if [[ "$CHANGE_REPO" =~ ^[Ss]$ ]]; then
        ...
        REMOTE_URL="" # Limpa a variável para forçar a configuração abaixo
    else
        echo -e "${GREEN}✅ Mantendo repositório atual.${NC}"
    fi
fi

# Se não tem remoto (ou se foi removido acima), pede a nova URL
if [ -z "$REMOTE_URL" ]; then
    echo -e "${CYAN}📌 PASSO 1/4: CONFIGURAÇÃO DO REPOSITÓRIO REMOTO${NC}"
    while true; do
        ...
        if [[ "$NEW_REPO_URL" =~ ^https://github.com/.*\.git$ ]]; then 
            ...
            echo -e "${GREEN}✅ Novo repositório conectado!${NC}"
            break
        fi
        echo -e "${RED}🚨 URL inválida. O link deve ser HTTPS e terminar em .git.${NC}"
    done
else
    # Apenas reforça a mensagem caso tenha mantido o antigo
    echo -e "${GREEN}✅ Conectado a: ${CYAN}$REMOTE_URL${NC}"
fi

# 2. OBTENÇÃO DE CREDENCIAIS (Para PULL e PUSH)
# ----------------------------------------------------------
while true; do
    echo -e "\n${CYAN}📌 PASSO 2/4: AUTENTICAÇÃO (TOKEN PAT)${NC}"
    echo -e "Use um Personal Access Token (PAT):"
    echo -e "1 - Digitar credenciais."
    echo -e "2 - ${CYAN}AJUDA: Como obter meu PAT?${NC}"
    read -r -p "$(echo -e "${YELLOW}Escolha a opção (1 ou 2): ${NC}")" AUTH_CHOICE
    
    if [ "$AUTH_CHOICE" == "1" ]; then
        echo -e "${RED}\n⚠️ ATENÇÃO: O token será visível!${NC}"
        read -r -p "$(echo -e "${YELLOW}👤 Nome de Usuário: ${NC}")" GIT_USERNAME
        read -r -p "$(echo -e "${YELLOW}🔑 Personal Access Token (PAT): ${NC}")" GIT_PASSWORD
        GIT_USERNAME_STORE="$GIT_USERNAME"; GIT_PASSWORD_STORE="$GIT_PASSWORD"
        break
    elif [ "$AUTH_CHOICE" == "2" ]; then
        echo -e "\n${CYAN}🔗 GUIA: [Criar um PAT](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/creating-a-personal-access-token)${NC}"
        read -p "$(echo -e "${YELLOW}Pressione [Enter] para retornar...${NC}")"
    fi
done
